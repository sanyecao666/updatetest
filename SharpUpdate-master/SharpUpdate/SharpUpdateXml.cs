...
namespace SharpUpdate
{
    ...
    public class SharpUpdateXml
    {
        ...
        {
            if (location.ToString().StartsWith("file"))//确定此字符串实例的开头是否与指定的字符串匹配
            {
        ...
        public static SharpUpdateXml[] Parse(Uri location)
        {
            ...
            try
            {
                ...
                string xmlret=location.AbsoluteUri;
				doc.Load(location.AbsoluteUri);//从指定的url加载xml
                ...
                foreach (XmlNode updateNode in updateNodes)
                {
                    ...
                    filePath = updateNode["fileName"].InnerText;//filePath
