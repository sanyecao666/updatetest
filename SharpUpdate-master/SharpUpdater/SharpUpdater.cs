...
namespace SharpUpdate
{
    ...
    public class SharpUpdater
    {
        ...
        public SharpUpdater(Assembly a, Window owner, Uri XMLOnServer)
        {
            ParentForm = owner;
            ParentAssembly = a;
			...
			if (!e.Cancelled)
			{
				...
				if (JobsFromXML != null)
				{
                    ...
                    for (int i = 0; i < Num_Jobs; ++i)
                    {
                        ...
                        {
                            LocalApplicationInfos[i] = new SharpUpdateLocalAppInfo(JobsFromXML[i], ParentAssembly, ParentForm);//替换
                        }
                        else
                        {
                            LocalApplicationInfos[i] = new SharpUpdateLocalAppInfo(JobsFromXML[i]);//新的增加的
                        }
        ...
        private void DownloadUpdate(SharpUpdateXml update, SharpUpdateLocalAppInfo applicationInfo)
        {
            ...
            //测试，取消MD5加密
            SharpUpdateDownloadForm form = new SharpUpdateDownloadForm(update.Uri,applicationInfo.ApplicationIcon);
        ...
        private void UpdateApplications()
        {
            ...
            //string argument_update = "/C choice /C Y /N /D Y /T 4 & Del /F /Q \"{0}\" & choice /C Y /N /D Y /T 2 & Move /Y \"{1}\" \"{2}\"";origion
            string argument_update = "/C choice /C Y /N /D Y /T 4 & Del /F /Q \"{0}\" & choice /C Y /N /D Y /T 2 & Move \"{1}\" \"{2}\"";
            //string argument_update_start = argument_update + " & Start \"\" /D \"{3}\" \"{4}\" {5}";//origion
            string argument_update_start = argument_update + " & Start \"{3}\" \"{4}\\{5}\"";
            //string argument_add = "/C choice /C Y /N /D Y /T 4 & Move /Y \"{0}\" \"{1}\"";//origion
            string argument_add = "/C choice /C Y /N /D Y /T 4 & Move  \"{0}\" \"{1}\"";
            ...
            if (curAppidx > -1)
            {
                ...
                //argument_complete = @"/C choice /C Y /N /D Y /T 4 & Del /F /Q ""D:\GoogleDownload\SharpUpdate - master\TestApp\bin\Debug\TestApp.exe"" & choice /C Y /N /D Y /T 2 & Move /Y @""C:\Users\Administrator\AppData\Local\Temp\tmpD2E2.tmp"" ""D:\GoogleDownload\SharpUpdate - master\TestApp\bin\Debug\TestApp.exe"" & Start @""D:\GoogleDownload\SharpUpdate - master\TestApp\bin\Debug\TestApp.exe""";
            }
            else
            {
